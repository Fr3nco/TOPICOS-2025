...
int vectorOrdInsertar(Vector* vector, const void* elem, Cmp cmp)
{
    ...
    {
        i += vector->tamElem; //es como i++ pero al ser void solo avanzaria un byte por eso avanza con el tañano del elemento
    }
    ...
    {
        memcpy(i + vector->tamElem, i, vector->tamElem);//Copia el elemento actual (i) a su derecha (i + tamaño), dejando i libre para el nuevo elemento".
    }
...
int vectorOrdBuscar(const Vector* vector, void* elem, Cmp cmp)
{
    ...
    memcpy(elem, m, vector->tamElem);//rellena elem con el valor de m

    return (m - vector->vec) / vector->tamElem;//devuelve la posicion
}
...
void ordenarBurbujeo(Vector* vector, Cmp cmp) {  // ¡Necesitas la función cmp!
    void* ult = vector->vec + (vector->ce - 1) * vector->tamElem;  // Último elemento.

    for(void* i = vector->vec; i < ult; i += vector->tamElem) {
        void* limJ = ult - (i - vector->vec);  // Límite para el bucle interno.

        for(void* j = vector->vec; j < limJ; j += vector->tamElem) {
            if(cmp(j, j + vector->tamElem) > 0) {  // Comparación genérica.
...
void* buscarMenor(void* ini, void* fin, Cmp cmp, size_t tamElem)
{
 ...
 void ordenarInsercion(Vector* vector,Cmp cmp)
 {
     ...
     for(void* i = vector->vec + vector->tamElem; i <= ult; i += vector->tamElem)
     {
        ...
        void* j = i - vector->tamElem;
        while(j >= vector->vec && cmp(elemAIns, j) < 0)
         {
